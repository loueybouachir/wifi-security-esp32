...
void app_main(void) {
    ...
    while (1) { 
        ...
        // Deauth attack target parameters
        uint8_t deauth_bssid[6] = {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA}; // AP BSSID
        uint8_t deauth_mac[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};   // Client MAC (broadcast)
        int deauth_channel = 1; // Channel 
        ...
        if (attack_running) {
            ...
            TickType_t last_deauth_time = xTaskGetTickCount(); 
